...
> * **专属中英术语对照表按钮 (Bilingual Terminology Table Button)**
>   * **特性描述**：在对象列表窗口（Objects）底部的常用固定按钮中，我们全新设计并集成了 **`对照表` (Bilingual)** 专属按钮。
>   * **功能作用**：点击该按钮能够一键弹出一个详尽、专业的“语音学及 Praat 数据结构中英文对照参考表”，按学科大类（核心语音声学、数据结构标注、高级DSP算法、语音合成及高级声带特征、优选论）和首字母字母表进行双重分类，方便在汉化使用中随时对照英文原词，降低学术交流阻碍。
> * **汉化帮助手册的深度体验优化 (Help Manual Chinese Translation & UX Optimizations)**
>   * **手册全面汉化**：完成了对 Praat 内置的核心帮助手册本身（Manual 及其子系统页面）的全方位的深度中文翻译，为国内学者和学习者提供友好的无障碍学术阅读环境。
>   * **动态自适应滚动条**：彻底重构了原版滚动条行为（摒弃了预设的长达 320 英寸的固定大空白设计）。如今能够根据页面排版和渲染的真实内容高度动态计算滚动滑块大小与翻页刻度，带来极度顺滑的拖拽及键盘翻页操作体验。
>   * **显示截断与窗口缩放警告修复**：完美修正了页面底部版权与原作者签名（© Paul Boersma...）下沉部字符截断的显示缺陷；同时解决了由于 Motif 底层非原子参数调整所致、在全屏切换或随意缩放窗口时极易反复弹出“XmScrollBar: value greater than maximum...”越界警告框的系统隐患，大幅提升了系统的运行稳定性与使用品质。
> 
